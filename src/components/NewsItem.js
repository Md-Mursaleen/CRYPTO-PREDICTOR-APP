...
import { StyleSheet, Text, View, Linking, Alert, Image, Pressable } from 'react-native';
...
const NewsItem = ({ item }) => {
    ...
    return (
        <Pressable onPress={() => openLink(url)} style={styles.newsItemContainer}>
            {thumb_2x && thumb_2x !== 'missing_large.png' && (
                <Image source={{ uri: thumb_2x }} style={styles.newsItemImageStyle} />
            )}
            <Text style={styles.newsTitleTextStyle}>{title} {' '}<Text onPress={() => openLink(url)} style={styles.newsLinkTextStyle}>Read more</Text></Text>
            <View style={styles.newsItemTextContainer}>
                <Text style={styles.newsTextStyle}>{news_site} • </Text>
                <Text style={styles.newsTextStyle}>{timeAgo}</Text>
            </View>
        </Pressable>
    );
}
